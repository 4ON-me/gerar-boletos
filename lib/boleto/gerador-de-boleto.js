	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				const linhaDigitavel = GeradorDeLinhaDigitavel(boleto, banco);
				...
				var tituloEspecieDoc = tituloNumeroDoDocumento + 110;
				pdf.font('negrito')
					.fontSize(args.tamanhoDaFonteDoTitulo)
				...
				var tituloAceite = tituloEspecieDoc + 66;
				pdf.font('negrito')
					.fontSize(args.tamanhoDaFonteDoTitulo)
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(boleto.getValorFormatadoBRL(), tituloValorMoeda, args.ajusteY + sextaLinha, {
						lineBreak: false,
						width: 294,
						align: 'left'
					});
				...
				informacoesPersonalizadas(pdf, args.ajusteX + margemDoSegundoBlocoLayout, args.ajusteY + linha212 + args.tamanhoDoCodigoDeBarras + 15);
	...
	GeradorDeBoleto.prototype.gerarLinhaDigitavel = function() {
		return new Promise((resolve)=> {
			...
			boletos.forEach((boleto, indice)=> {
				...
				const linha = GeradorDeLinhaDigitavel(boleto, banco);
				if (indice <= boletos.length - 1) {
